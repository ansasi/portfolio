...
- [x] A1. Define a neutral palette in `src/index.css` (replace the indigo accent
  and blue-tinted neutrals — see palette table below).
- [x] A2. Swap both logo gradients (navbar + footer, `#033392 → #027add`) for
  solid black / mono.
- [x] A3. Re-tone the floating navbar glass (`rgba(26, 89, 157, .3)`) and the
  nav-dot active color to neutral.
- [x] A4. Hero CTAs → black / outline buttons with neutral shadows
  (drop the indigo glow `rgba(49, 59, 172, …)`).
- [x] A5. Portfolio filters + Contact form/button (incl. hover `#2430af`) → black.
- [x] A6. Tinted card backgrounds (`#fef4f5`, `#f2f7fb`, `#edf2f8`) → true greys.
- [x] A7. Hero / mobile-menu background textures (`bgIMG.png`, `bgWhite.png`)
  carry color — replace with a neutral pattern or remove.
- [x] A8. Keep imagery full color (confirm no stray filters once the theme lands).
...
- [x] B9. **Fix the broken Footer CSS variables** — `src/components/Footer/Footer.scss`
  ...
  never apply.
- [x] B10. Remove the dead `src/container/Experience/Experience.scss` (~125 lines
...
- [x] B11. Replace remaining hardcoded hex with CSS variables
...
- [x] C12. Portfolio cards are `width: 40%` + margins (the same "wasted desktop
  space" the About grid had) → convert to a responsive grid; fix the overlapping
  floating tag.
- [x] C13. Theme the Experience `VerticalTimeline` to match the new tone
  (currently the library default look + inline white).
- [x] C14. Contact: tidy the form container and add `<label>`s.

---

## D. Polish & accessibility

- [x] D15. Unify card radius / shadow / padding into shared tokens; settle on one
  consistent button style across hero, contact, and portfolio filters.
- [x] D16. Add `aria-current` to the active nav item, labels / `aria-label` to
  form inputs, and verify muted-grey text contrast (WCAG AA).
- [x] D17. Ensure the hamburger menu works below 500px (today it only shows
